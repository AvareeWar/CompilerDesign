 ...
 *---	implement the unenhanced llParser as defined by 		---*
 *---									---*
 *---									---*
 *---	----	----	----	----	----	----	----	----	---*
 *---									---*
			...
			throw()
  {
    for  (uInt pi = 0;  pi < getNumProductions();  pi++)
    {
      ...
      if  (prodPtr->getLhsPtr() == getStartSymbolPtr())
      	prodPtr->addSymbolAtEnd(symbolTable_.find(END_SYMBOL));
    }
  }
...
// {if,then,else,endif,var,;,+},{Stmt,StmtList,Expr},Stmt,{Stmt - if Expr then StmtList endif, Stmt - if Expr then StmtList else StmtList endif, StmtList - StmtList ; Stmt, StmtList - Stmt, Expr - var + Expr, Expr - var}
